...
class PsiFormer(nn.Module):
    ...
    @nn.compact
    def __call__(self, electron_nuclear_features: jnp.ndarray,
                 single_electron_features: jnp.ndarray) -> jnp.ndarray:
        """
        ...
        x = nn.Dense(features=self.num_heads,
                     use_bias=False,
        ...
        for _ in range(self.num_of_blocks):
            x = PsiFormerBlock(num_heads=self.num_heads,
                               use_memory_efficient_attention=self.use_memory_efficient_attention,
                               group=self.group,
                               param_dtype=self.param_dtype,
                               computation_dtype=self.computation_dtype)(x)

        psiformer_pre_det = nn.Dense(features=self.num_of_electrons * self.num_of_determinants,
                                     use_bias=False,
                                     dtype=self.computation_dtype,
                                     param_dtype=self.param_dtype
                                     )(x)

        summed_dets = Envelop(num_of_determinants=self.num_of_determinants,
                              num_of_electrons=self.num_of_electrons,
                              num_of_nucleus=self.num_of_nucleus,
                              param_dtype=self.param_dtype,
                              computation_dtype=self.computation_dtype
                              )(jnp.expand_dims(electron_nuclear_features[..., -1], -1),
                                psiformer_pre_det)
