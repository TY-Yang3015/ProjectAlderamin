...
_MAT_INV_PTH_ROOT_DTYPE = jnp.float32


def _default_zero_field():
    return struct.field(default_factory=functools.partial(jnp.array, 0, jnp.float32))
...
class InversePthRootDiagnostics:
    ...
    def create(cls, pth_inverse_root, matrix, p):
        ...
        mat_m = jnp.matmul(
            mat_power(pth_inverse_root, p), matrix, precision=jax.lax.Precision.HIGHEST
        )
        num_off_diag_entries = mat_m.size - jnp.diag(mat_m).size
        diag_error = jnp.abs(jnp.diag(mat_m) - 1).astype(jnp.float32)
        off_diag_error = jnp.abs(mat_m - jnp.diag(jnp.diag(mat_m))).astype(jnp.float32)
        return cls(
            ...
            avg_off_diag_error=(jnp.sum(off_diag_error) / num_off_diag_entries).astype(
                jnp.float32
            ),
...
class TrainingMetrics:
    ...
    conditioned_inverse_pth_root_diagnostics: InversePthRootDiagnostics = struct.field(
        default_factory=InversePthRootDiagnostics
    )
...
def init_training_metrics(num_statistics, generate_training_metrics):
...
def init_training_metrics_shapes(num_statistics, generate_training_metrics):
...
def matrix_inverse_pth_root(
    ...
    def _iter_body(state):
        ...
        error_ratio = jnp.float32(new_error / error)
        return (i + 1, new_mat_m, new_mat_h, mat_h, new_error, error_ratio)
    ...
    if matrix_size == 1:
        ...
        init_state = tuple([0, new_mat_m_0, new_mat_h_0, new_mat_h_0, new_error, 1])
...
def _add_metrics_into_local_stats(local_stats, metrics, keep_old):
    ...
    for local_stat in local_stats:
        ...
        new_local_stats.append(local_stat.replace(training_metrics=per_stat_metrics))
...
def distributed_shampoo(
        ...
        for param in params_flat:
            ...
            local_stats_flat.append(
                LocalShardedParameterStats(
                    ...
                    init_training_metrics_shapes(len(sizes), generate_training_metrics),
                    index_start,
                    sizes,
                )
            )
    ...
    def _quantized_matrix_inverse_pth_root_vmap(qxs, qds, qbs, ps, padding_starts):
        ...
        return jax.vmap(matrix_inverse_pth_root_wrapper)(
            qxs, qds, qbs, ps, padding_starts
        )

    def _matrix_inverse_pth_root_pjit(
        xs, ps, padding_starts, statistics_partition_spec=None
    ):
        ...
        (
            partitioned_preconditioners,
            partitioned_metrics,
        ) = _matrix_inverse_pth_root_vmap(
            partitioned_xs, partitioned_ps, partitioned_padding_starts
        )
        ...
        metrics = pjit.with_sharding_constraint(
            partitioned_metrics, pjit.PartitionSpec()
        )
        return preconditioners, metrics
    ...
    def _pmap_compute_preconditioners(
        ...
        for p, shape, prev_p, error in zip(
            preconditioners_flat, original_shapes, prev_preconditioners, new_errors_flat
        ):
        ...
        for num_statistics, state in zip(num_statistics_per_state, states):
            if num_statistics == 0:
                ...
                if generate_training_metrics:
                    ...
                    assert len(state.statistics) == len(
                        quantized_preconditioners_for_state
                    )
    ...
    def _pjit_compute_preconditioners(
        ...
        for p, shape, prev_p, error in zip(
            ...
            new_preconditioners_flat.append(
                _select_preconditioner(
                    error.inverse_pth_root_errors, p[: shape[0], : shape[1]], prev_p
                )
            )
